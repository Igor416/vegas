...
import { getBestProducts, getMattressColectionsPrice } from '../api';
import { BestProducts, MattressColectionPrice, Price } from '../JSONTypes';
  ...
  return (
    <div id='header' style={{zIndex: 1200}} className='bg-white'>
      ...
      <div style={{boxShadow: '0 1rem 1.5rem -.5rem rgba(0, 0, 0, .25)'}} className='container-fluid row p-3 align-items-center m-0'>
        ...
        {langs().map((l, index) => {
        return (
          <button key={index} onClick={() => updateLang(l)} className='p-2 bg-white border-0 outline-0 no-hover h4'>
            <span className='border-bottom'>{l}</span> 
          </button>
        )})}
        </div>
