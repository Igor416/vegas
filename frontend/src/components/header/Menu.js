      ...
      >
        ...
        >
          ...
          return (
            ...
            >
              <CustomLink link={get_link(sub_category)} text={sub_category.split(';')[0]} isActive={state.sub_category == sub_category} />
            </div>
          )})}
        </div>
        <div
        ...
        >
          {state.sub_category && categories[state.category][state.sub_category].map((link, index) => {
          return (
            <div
              className="d-flex pb-2"
              key={index}
              onMouseEnter={() => props.onMouseEnter(true, state.category, state.sub_category, true, link)}
              onMouseLeave={() => props.onMouseLeave(true, state.sub_category, true, link)}
            >
              <CustomLink link={get_link(state.sub_category)} text={link} isActive={state.link == link} menu_link={true} />
            </div>
          )})}
        </div>
