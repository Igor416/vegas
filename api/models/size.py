from uuid import uuid4

from django.db import models
from django.utils.functional import cached_property

from .product import Product


class SizeManager(models.Manager):
    def get_queryset(self):
        return (
            super()
            .get_queryset()
            .annotate(
                discounted_price=models.ExpressionWrapper(
                    models.F("priceMDL") * (100 - models.F("discount")) / 100.0,
                    output_field=models.FloatField(),
                )
            )
            .order_by(
                models.F("on_sale").desc(),
                models.F("product__category__order"),
                models.F("product__name_en"),
                "discounted_price",
            )
        )


class Size(models.Model):
    id = models.UUIDField("ID", primary_key=True, default=uuid4)

    width = models.SmallIntegerField("Ширина", default=80)
    length = models.SmallIntegerField("Длина", default=200)
    priceMDL = models.SmallIntegerField("Цена (MDL)", default=0)
    discount = models.SmallIntegerField("Скидка (%)", default=0)
    on_sale = models.BooleanField("На распродаже", default=False)
    out_of_stock = models.BooleanField("Отсуствует", default=False)
    product = models.ForeignKey(
        Product,
        on_delete=models.SET_NULL,
        blank=True,
        null=True,
        related_name="sizes",
        verbose_name="Товар",
    )

    objects = SizeManager()
    disabled = models.BooleanField("Отключен", default=False)

    def __str__(self):
        return f"Размер продукта {self.product}: {self.width} x {self.length} по цене {self.priceMDL} (MDL){f', со скидкой {self.discount}%' if self.discount != 0 else ''}{' (распродажа)' if self.on_sale else ''}"

    @cached_property
    def priceEUR(self):
        return round(self.priceMDL / 20.5, 2)

    @cached_property
    def priceRON(self):
        return round(self.priceMDL / 4.1, 2)

    @cached_property
    def priceUSD(self):
        return round(self.priceMDL / 18.65, 2)

    class Meta:
        verbose_name = "Размер"
        verbose_name_plural = "Размеры"
