...
class DetailedProductView(RetrieveAPIView):
    ...
    def get(self, request, category, name):
        product = get_object_or_404(
            self.queryset, category__name=category, name_en=name.replace("%20", " ")
        )
        ...
        mapping = {
            "B": (
                product.boolean_characteristics.all(),
                BooleanCharacteristicSerializer,
            ),
            "I": (
                product.integer_characteristics.all(),
                IntegerCharacteristicSerializer,
            ),
