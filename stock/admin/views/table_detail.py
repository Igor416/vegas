...
class TableDetailView(PermissionRequiredMixin, DetailView, UpdateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context = self.get_context_data(
            *args, object=self.object, request=request, **kwargs
        )
        ...
        for entry in obj.stockables.filter(category=category):
            ...
            if entry.current_state == "S":
                ...
                if data["places"][entry.current_place] == "":
                    data["places"][entry.current_place] = (
                        0 if entry.current_state == "H" else 1
                    )
                else:
                ...
                if entry.current_state == "O":
                    data["total"] = (
                        ...
                        + "+"
                        + str(int(total.split("+")[1]) + 1)
                    )
                else:
                    data["total"] = (
                        str(int(total) + 1)
                        if "+" not in total
                        else str(int(total.split("+")[0]) + 1)
                        + "+"
                        + total.split("+")[1]
                    )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            stockables = Stockable.objects.filter(
                category=category,
                product=product,
                width=width,
                length=length,
            ...
            if place == "add" or not stockables.exists():
                for _ in range(int(prev)):
                    stockable = Stockable.objects.create(
                        category=category,
                        product=product,
                        width=width,
                        length=length,
                ...
                value_s, value_o = (
                    map(int, (x.strip() for x in value.split("+")))
                    if "+" in value
                    else (int(value.strip() if value != "" else 0), 0)
                )
                prev_s, prev_o = (
                    map(int, (x.strip() for x in prev.split("+")))
                    if "+" in prev
                    else (int(prev.strip() if prev != "" else 0), 0)
                )
                ...
                if place == "total":
                    ...
                    if value_s < prev_s:
                        for _ in range(prev_s - value_s):
                            if is_place:
                                cond = lambda s: (
                                    s.current_state == "S" or s.current_place != place
                                )
                            else:
                                cond = lambda s: (
                                    s.current_state != "S"
                                    or s.last_update.day != int(place)
                                )
                            while cond(stockables[i]):
                                i += 1
                            stockables[i].actions.add(
                                create_action("H" if is_place else "R", stockables[i])
                            )
                            i += 1
                    else:
                        for _ in range(value_s - prev_s):
                            ...
                            if is_place:
                                ...
                                a.update(
                                    type="S",
                                    date=datetime(table.year, table.month, int(place)),
                                )
                            i += 1
