...
def parse():
    ...
    wb = load_workbook(
        r"C:\Users\User\Downloads\Telegram Desktop\оба два (Автосохраненный).xlsx",
        data_only=True,
    )
    ws = wb.active
            ...
            if found.exists():
                ...
                for place in range(4, 10):
                    ...
                    for _ in range(count):
                        ...
                        s = Stockable.objects.create(
                            product=name, width=width, length=length, table=hidden_table
                        )
                        s.actions.add(
                ...
                if "+" in total:
                    ...
                    for i in range(ordered):
                        in_stock[i].actions.add(
                            Action.objects.create(
                                type="O",
                                person="admin",
                                place=places[4],
                                stockable=in_stock[i],
                            )
                        )
